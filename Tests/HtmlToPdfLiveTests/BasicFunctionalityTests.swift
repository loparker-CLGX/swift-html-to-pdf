...
struct BasicFunctionalityTests {
  ...
  func testPDFWithTitle() async throws {
    try await withTemporaryDirectory { directory in
      ...
      #expect(
        ...
        "PDF with correct filename should exist"
      )
    }
  }
  ...
  func testCustomConfiguration() async throws {
    try await withTemporaryPDF { output in
      ...
      #expect(
        FileManager.default.fileExists(atPath: result.path),
        "PDF with custom config should be created"
      )
    }
  }
  ...
  func testComplexHTML() async throws {
    try await withTemporaryPDF { output in
      ...
      #expect(
        FileManager.default.fileExists(atPath: result.path),
        "Complex HTML PDF should be created"
      )
  ...
  func testBaseURLConfiguration() async throws {
    try await withTemporaryPDF { output in
      ...
      #expect(
        FileManager.default.fileExists(atPath: result.path),
        "PDF with baseURL should be created"
      )
    }
  }
  ...
  func testUSLetterPaperSize() async throws {
    try await withTemporaryPDF { output in
      ...
      #expect(
        FileManager.default.fileExists(atPath: result.path),
        "US Letter PDF should be created"
      )
    }
  }
  ...
  func testMinimalMargins() async throws {
    try await withTemporaryPDF { output in
      ...
      #expect(
        FileManager.default.fileExists(atPath: result.path),
        "PDF with minimal margins should be created"
      )
    }
  }
}
