...
@Suite("Performance Benchmarks", .serialized, .tags(.benchmark)) struct PerformanceBenchmarks {
            ...
            for batch in 1...10 {
                ...
                group.addTask { @Sendable in
                    try? await withDependencies {
                        $0.pdf.render.configuration.namingStrategy = .init { i in
                            "batch\(batch)-doc\(i)"
                        }
                    } operation: {
                        ...
                        var urls: [URL] = []
                        for try await result in try await batchPdf.render.client.html(
                            html,
                            to: outputDir
    ...
    func generateReadmeTable() async throws {
        ...
        print(
            "- **Choose Continuous** for: Web captures, articles, infographics (single tall page)"
        )
        print(
    ...
    @Test("Concurrency sweep to find optimal level") func concurrencySweep() async throws {
        ...
        for concurrency in levels {
            try await withDependencies {
                ...
                print(
                    "✓ Concurrency \(String(format: "%2d", concurrency)):  \(String(format: "%4.0f", throughput)) PDFs/sec  (\(String(format: "%.2f", seconds))s)"
                )
