...
@Suite("Cancellation Tests", .serialized) struct CancellationTests {
    ...
    @Test("Cancelled task does not produce output file") func testCancelledTaskNoOutput()
        async throws
    {
        try await withTemporaryPDF { output in
    ...
    @Test("Batch stream stops when task is cancelled") func testBatchStreamCancellation()
        async throws
    {
        await withTemporaryDirectory { dir in
