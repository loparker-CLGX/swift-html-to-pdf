...
@Suite("Error Handling Tests", .serialized) struct ErrorHandlingTests {
    ...
    @Test(
        ...
        do {
            ...
            // Expected to fail
            #expect(
                error.localizedDescription.contains("exist")
    ...
    @Test(
        ...
        try await withTemporaryDirectory { output in
            let html = "<html><body>Test</body></html>"
            let nestedPath = output.appendingPathComponent("nested").appendingPathComponent(
                "directories"
        ...
        async throws
    {
        try await withTemporaryDirectory { output in
            ...
            let specialNames = [
                ...
                "test<with>brackets", "test|with|pipes", "test*with*asterisks",
                "test\"with\"quotes",
            ]
            ...
            #expect(
                files.count == specialNames.count,
                "All files should be created with sanitized names"
            )
        }
    }
}
