...
//
  ...
  @globalActor private actor WebViewPoolActor {
    ...
    func getOrCreatePool(
      ...
      if let existing = sharedPool { return existing }
  ...
  package struct WebViewPoolClient: Sendable {
    ...
    package var pool: ResourcePool<WKWebViewResource> { get async throws { try await getPool() } }
  }

  extension WebViewPoolClient: DependencyKey {
    public static var liveValue: WebViewPoolClient {
      return WebViewPoolClient(poolProvider: { @MainActor in
