...
public actor LiveMetricsDisplay {
    ...
    private func printCurrentMetrics() async {
        ...
        print(
            "📊 Live Metrics | PDFs: \(pdfsGenerated) | Failed: \(pdfsFailed) | Pool: \(Int(poolUtil)) | Throughput: \(String(format: "%.0f", throughput))/sec | p95: \(String(format: "%.1f", p95))ms"
        )
    }
}
...
public struct MetricsComparison: Sendable {
    ...
    public func summary() -> String {
        ...
        if latencyRegression > tolerance {
            lines.append(
                "    ⚠️  REGRESSION: +\(String(format: "%.1f", latencyRegression * 100))% (tolerance: \(String(format: "%.1f", tolerance * 100))%)"
            )
        }
        lines.append(
        ...
        if throughputRegression > tolerance {
            lines.append(
                "    ⚠️  REGRESSION: -\(String(format: "%.1f", throughputRegression * 100))% (tolerance: \(String(format: "%.1f", tolerance * 100))%)"
            )
        }
...
public func compareMetrics(
    ...
    let currentThroughput =
        totalDuration > 0
        ? Double(pdfsGenerated) / (TimeInterval(totalDuration) / 1_000_000_000) : 0
